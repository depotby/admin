...
import type { CategoryData } from '@/types/models/category.ts';
...
useHead(() => ({
  title: t('pages.categories.new.meta.title'),
}));
</script>

<template>
  <div :class="$style['page-categories-new']">
    <UiText variant="h3">
      {{ $t('pages.categories.new.title') }}
    </UiText>

    <form :class="$style['page-categories-new__form']" @submit.prevent="createCategory">
      <UiFormInput
        v-model="form.name"
